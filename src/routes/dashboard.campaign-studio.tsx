...
function AbVariantsSection({ variants, campaignId }: { variants: CampaignBrief["abVariants"]; campaignId?: string }) {
  ...
  return <section className="rounded-lg border border-border bg-background p-4"><h3 className="font-extrabold">اختر أقوى رسالة تسويقية</h3><div className="mt-3 grid gap-3">{variants.map((v, index) => { const text = `${v.hook}\n${v.message}\n${v.cta}`; return <article key={`${v.style}-${index}`} className="rounded-lg border border-border bg-card p-3"><div className="flex items-center justify-between gap-2"><p className="font-extrabold"><span aria-hidden="true">{v.icon}</span> {v.style}</p><Button size="sm" variant="ghost" onClick={() => { void navigator.clipboard.writeText(text); toast.success("تم نسخ النسخة"); }}><Copy className="h-3.5 w-3.5" /></Button></div><p className="mt-2 text-sm font-bold leading-6">{v.hook}</p><p className="mt-1 text-xs leading-6 text-muted-foreground">{v.message}</p><Button asChild size="sm" variant="outline" className="mt-3 h-8 text-xs"><Link to="/dashboard/generate-text" search={campaignExecutionSearch({ campaignId }, text) as never}>استخدمها في النص</Link></Button></article>; })}</div></section>;
}
...
function PublishingCalendarSection({ days, campaignId }: { days: CampaignBrief["publishingCalendar"]; campaignId?: string }) {
  ...
  return <section className="rounded-lg border border-border bg-background p-4"><div className="flex items-center gap-2"><CalendarDays className="h-4 w-4 text-primary" /><h3 className="font-extrabold">تقويم نشر 7 أيام</h3></div><div className="mt-3 grid gap-3 sm:grid-cols-2">{days.map((day) => { const text = `${day.contentType}\n${day.message}\nالهدف: ${day.goal}`; return <article key={day.day} className="rounded-lg border border-border bg-card p-3"><div className="flex items-center justify-between gap-2"><Badge variant="outline">اليوم {day.day} · {day.label}</Badge><span className="text-[11px] font-bold text-primary">{day.channel}</span></div><p className="mt-2 text-sm font-extrabold">{day.contentType}</p><p className="mt-1 text-xs leading-6 text-muted-foreground">{day.message}</p><p className="mt-2 text-[11px] font-bold text-muted-foreground">الهدف: {day.goal}</p><div className="mt-3 flex flex-wrap gap-2"><Button size="sm" variant="ghost" className="h-8 text-xs" onClick={() => { void navigator.clipboard.writeText(day.message); toast.success("تم نسخ رسالة اليوم"); }}><Copy className="h-3.5 w-3.5" /> نسخ</Button><Button asChild size="sm" variant="outline" className="h-8 text-xs"><Link to="/dashboard/generate-text" search={campaignExecutionSearch({ campaignId, channel: day.channel }, text) as never}>حوّلها لمحتوى</Link></Button></div></article>; })}</div></section>;
}
